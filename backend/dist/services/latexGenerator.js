...
exports.compileLatexSource = exports.generateLatex = void 0;
...
const buildResumeHtml = (data) => {
    ...
    const skillsRows = Object.entries(skillsObj)
        ...
        .map(([cat, items]) => `<tr><td class="skill-label">${e(cat)}</td><td>${items.map(e).join(', ')}</td></tr>`).join('');
    ...
    const experienceBlocks = (data.experience || []).map((exp) => `
        <div class="exp-block">
            ...
            <ul>${(exp.bullets || []).map((b) => `<li>${e(b)}</li>`).join('')}</ul>
        </div>`).join('');
    const projectItems = (data.projects || []).map((proj) => `
        <li><strong>${e(proj.name || proj.title)}.</strong> ${e(proj.description || (proj.bullets || []).join(' '))}</li>`).join('');
    const extraItems = (data.extracurriculars || []).map((ex) => `<li>${e(ex)}</li>`).join('');
    const leadershipItems = (data.leadership || []).map((l) => `<li>${e(l)}</li>`).join('');
    const section = (title, content) => content.trim() ? `<div class="section"><p class="section-title">${title}</p><hr/>${content}</div>` : '';
    // Contact line
    const contact1 = [data.phone, data.location].filter(Boolean).map(e).join(' &nbsp;◇&nbsp; ');
    const contact2Parts = [];
    if (data.email)
        contact2Parts.push(`<a href="mailto:${e(data.email)}">${e(data.email)}</a>`);
    if (data.linkedin)
        contact2Parts.push(`<a href="https://${e(data.linkedin)}">${e(data.linkedin)}</a>`);
    if (data.website)
        contact2Parts.push(`<a href="https://${e(data.website)}">${e(data.website)}</a>`);
...
// ─── Public API ───────────────────────────────────────────────────────────────
const generateLatex = (resumeData) => __awaiter(void 0, void 0, void 0, function* () {
...
const compileLatexSource = (htmlSource) => __awaiter(void 0, void 0, void 0, function* () {
    const pdfPath = path.join(os.tmpdir(), `resume_${Date.now()}.pdf`);
    const browser = yield puppeteer_1.default.launch({
        headless: true,
        args: ['--no-sandbox', '--disable-setuid-sandbox'],
    ...
    try {
        const page = yield browser.newPage();
        yield page.setContent(htmlSource, { waitUntil: 'networkidle0' });
        yield page.pdf({
            path: pdfPath,
            format: 'Letter',
            printBackground: true,
...
exports.compileLatexSource = compileLatexSource;
